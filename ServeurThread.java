...
public class ServeurThread implements Runnable{
  ...
  public void LOGIN(BufferedReader br, PrintWriter pw){
    try{
      ...
      PublicKey k = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(decoder.decode(key)));
      String end = br.readLine();
      if(!end.equals("***")){
        pw.print("ERROR\nWRONG_ARGUMENT\n***\n");
        pw.flush();
      ...
      for(int i =0;i<this.s.users.size();i++){
        if(this.s.users.get(i).get_username().equals(username)){
          ...
          this.u.setRSA(k);               //On met à jour la clé RSA
  ...
  public void ADDDOMAIN(BufferedReader br, PrintWriter pw){
    try{
      ...
      if(this.u == null){
        ...
        pw.flush();
        return;
      }if(!this.u.is_superuser()){
        pw.println("ERROR\nUSER_NOT_ALLOWED\n***\n");
        pw.flush();
        return;
      }
      for(int i =0;i<this.s.products.size();i++){
        if(this.s.products.equals(name)){
          pw.println("ERROR\nDOMAIN_ALREADY_EXISTS\n***\n");
          pw.flush();
          return;
        }
      }
      Domaine d = new Domaine(name);
      this.s.products.add(d);
      pw.println("OK\n***\n");
      pw.flush();
    }catch(Exception e){e.printStackTrace();}
  }
        ...
        for(int j = 0;j<this.s.users.get(i).get_articles().size();j++){
          if(ip_int == this.s.users.get(i).get_articles().get(j).get_ref()){
            if(this.s.users.get(i).is_connected()){
              String key_pub = Base64.getEncoder().encodeToString(this.s.users.get(i).getRSA().getEncoded());
 ...
 }catch(Exception e){System.out.println("User Quit");}
 }
}
